/* Auto-generated by scripts/build-n8n.js
 * Date: 2026-02-20T04:50:02.460Z
 * Contains PDL JS library + n8n adapter.
 */

// ===== PDL node config (inputs) =====
const pdlConfig = {
...
const PDL = {
  ...
  DATE_INVALID_DEFAULT: 'Invalid date',
  DEFAULT_TIME_EMPTY: 'Invalid time',
};
...
function coerceNumber(x) {
  ...
  if (typeof x === 'string') {
    const s = x.trim();
    if (/^-?\d+([eE][+-]?\d+)?$/.test(s)) {
      try { return Number.parseInt(s, 10); } catch { return null; }
    }
    if (/^-?\d*\.\d+([eE][+-]?\d+)?$/.test(s)) {
      try { return Number.parseFloat(s); } catch { return null; }
    }
  }
  return null;
}
...
function formatDurationMs(ms, schema, { empty }) {
  const n = coerceNumber(ms);
  if (n == null) return empty;
  let totalMs;
  try { totalMs = Math.trunc(n); } catch { return empty; }
  if (totalMs < 0) return empty;

  const wantsY = (schema || '').includes('%Y');
  const wantsMo = (schema || '').includes('%M');
  const wantsD = (schema || '').includes('%D');
  const wantsH = (schema || '').includes('%h');
  const wantsM = (schema || '').includes('%m');
  const wantsS = (schema || '').includes('%s');
  const wantsL = (schema || '').includes('%l');
  ...
  const hasDayOrBigger = wantsD || wantsMo || wantsY;
  const hasHourOrBigger = hasDayOrBigger || wantsH;
  const hasMinOrBigger = hasHourOrBigger || wantsM;
  const hasSecOrBigger = hasMinOrBigger || wantsS;

  const valY = years;
  const valMo = months;
  const valD = days;
  const valH = wantsH ? (hasDayOrBigger ? remH : totalHours) : 0;
  const valM = wantsM ? (hasHourOrBigger ? remM : totalMinutes) : 0;
  const valS = wantsS ? (hasMinOrBigger ? remS : totalSeconds) : 0;
  const valL = wantsL ? (hasSecOrBigger ? remMs : totalMs) : 0;

  let outParts = [];
  const s = String(schema || '');
  for (let i = 0; i < s.length; i++) {
    const ch = s[i];
    if (ch !== '%') { outParts.push(ch); continue; }
    if (i + 1 >= s.length) { outParts.push('%'); continue; }
    const t = s[i + 1];
    if (t === '%') { outParts.push('%'); i++; continue; }
    const emit = (num, label) => outParts.push(`${num} ${plural(num, label)}`);

    if (t === 'Y') emit(valY, 'year');
    else if (t === 'M') emit(valMo, 'month');
    else if (t === 'D') emit(valD, 'day');
    else if (t === 'h') emit(valH, 'hour');
    else if (t === 'm') emit(valM, 'minute');
    else if (t === 's') emit(valS, 'second');
    else if (t === 'l') emit(valL, 'millisecond');
    else outParts.push('%' + t);

    i++;
  }

  const rendered = outParts.join('');
  ...
  for (let k = 0; k < tokens.length; ) {
    const a = tokens[k];
    const b = tokens[k + 1];
    if (b && unitWords.has(b) && /^-?\d+$/.test(a)) {
      chunks.push({ isUnit: true, num: parseInt(a, 10), word: b });
      k += 2;
    } else {
      chunks.push({ isUnit: false, lit: tokens[k] });
      k += 1;
    }
  }
  ...
  for (let idx = 0; idx < chunks.length; idx++) {
    if (chunks[idx].isUnit && chunks[idx].num !== 0) nz.push(idx);
  }
  if (!nz.length) return empty;

  const firstNz = nz[0];
  const lastNz = nz[nz.length - 1];

  const filtered = [];
  for (let idx = 0; idx < chunks.length; idx++) {
    const c = chunks[idx];
    if (c.isUnit) {
      if (c.num === 0 && (idx < firstNz || idx > lastNz)) continue;
      filtered.push(`${c.num} ${c.word}`);
    } else {
      filtered.push(String(c.lit));
    }
  }

  return filtered.join(' ').replace(/\s{2,}/g, ' ').trim() || empty;
}
...
function applyReplace(s, spec) {
  ...
  if (spec.startsWith('s/')) {
    const m = spec.match(/^s\/((?:\\.|[^/])*)\/((?:\\.|[^/])*)\/([imsIMS]*)$/);
    if (!m) return str;
    const patRaw = m[1].replace(/\\\//g, '/');
    const replRaw = m[2].replace(/\\\//g, '/');
    const flagsRaw = m[3] || '';
    let flags = '';
    if (flagsRaw.toLowerCase().includes('i')) flags += 'i';
    if (flagsRaw.toLowerCase().includes('m')) flags += 'm';
    if (flagsRaw.toLowerCase().includes('s')) flags += 's';
...
function parseScalarOrJson(raw) {
  ...
  if (s === 'null') return null;
  if (/^-?\d+(\.\d+)?([eE][+-]?\d+)?$/.test(s)) {
    try { return (s.includes('.') || /[eE]/.test(s)) ? Number.parseFloat(s) : Number.parseInt(s, 10); } catch { return s; }
  }
  return s;
}
...
class Scope {
  ...
  setVar(name, value, {
    ...
    let targetFrameIndex = (scope_local && this.var_frames.length > 0) ? (this.var_frames.length - 1) : 0;
    const [nearestBinding, nearestIdx] = this.getVarBinding(name);

    if (value == null) {
      if (nearestBinding && nearestIdx != null) delete this.var_frames[nearestIdx][name];
      return;
    }
...
class PathResolver {
  ...
  _eval(path, root, { default_ci } = {}) {
    ...
    for (const m of it) {
      ...
      if (cur && typeof cur === 'object' && !Array.isArray(cur) && Object.prototype.hasOwnProperty.call(cur, name)) {
        cur = cur[name];
      } else {
        return null;
      }
  ...
  _parseValueToken(tok) {
    ...
    if (/^-?\d+(\.\d+)?([eE][+-]?\d+)?$/.test(t)) {
      try { return (t.includes('.') || /[eE]/.test(t)) ? Number.parseFloat(t) : Number.parseInt(t, 10); } catch { return t; }
    }
  ...
  static strip(text) {
    ...
    for (let line of lines) {
      ...
      if (line.includes('//')) {
        ...
        if (!before.includes('://')) line = before.replace(/\s+$/, '');
      }
      out.push(line);
    }
    return out.join('\n');
...
class InlineSetDirective {
  apply(line, scope, resolver, stats) {
    ...
    while (true) {
      ...
      if (hasValue) {
        const [binding] = scope.getVarBinding(name);
        const allowFlipUpdate = Boolean(binding && binding.const && hadConst && constFlag === false);
        ...
        const [binding] = scope.getVarBinding(name);
        if (binding) {
          if (hadConst) binding.const = Boolean(constFlag);
          if (hadHumble) binding.humble = Boolean(humbleFlag);
        }
      }
...
class InlineGetDirective {
  apply(line, scope, resolver, stats) {
    let s = String(line);
    while (true) {
      const a = s.indexOf(PDL.GET_PREFIX);
      if (a === -1) break;
      ...
      const innerRaw = s.slice(a + PDL.GET_PREFIX.length, end).trim();
      const splitIdx = (() => {
      ...
      const name = (splitIdx === -1) ? innerRaw : innerRaw.slice(0, splitIdx).trim();
      const paramsRaw = (splitIdx === -1) ? '' : innerRaw.slice(splitIdx).trim();

      const params = parseKvFlags(paramsRaw, {
        types: {
          markdown: Boolean, trim: Boolean,
      ...
      const original = VAR_NAME_RE.test(String(name || '')) ? scope.getVarValue(name) : null;
      let valueOut = original;

      const timeSchema = params.time;
      if (timeSchema != null && timeSchema !== '') {
        let emptyTime = params.empty;
        if (emptyTime == null || emptyTime === '') emptyTime = PDL.DEFAULT_TIME_EMPTY;
        const n = coerceNumber(valueOut);
        if (n == null) valueOut = String(emptyTime);
        else valueOut = formatDurationMs(n, String(timeSchema), { empty: String(emptyTime) });
      }

      if (typeof valueOut === 'string') {
        let sText = valueOut;
        if (params.replace != null && params.replace !== '') sText = applyReplace(sText, String(params.replace));
        if (!Boolean(params.markdown)) sText = markdownEscape(sText);
        if (Boolean(params.trim)) sText = sText.trim();

        sText = applyCaseBasic(sText, { upper: !!params.upper, lower: !!params.lower, title: !!params.title });
        if (params.lowerCamel) sText = camelCase(sText, false);
        if (params.upperCamel) sText = camelCase(sText, true);
        if (params.lowerSnake) sText = snakeCase(sText, false);
        if (params.upperSnake) sText = snakeCase(sText, true);

        const trunc = params.truncate;
        if (Number.isInteger(trunc) && trunc > 0) sText = applyTruncate(sText, trunc, params.suffix);

        valueOut = sText;
      }

      let finalOut = toRenderText(valueOut, Boolean(params.stringify));
      if (finalOut == null) finalOut = '';

      s = s.slice(0, a) + finalOut + s.slice(end + 1);
      stats.expansions++;
    }
    return s;
  }
...
function expandValuesAndGetInline(text, scope, resolver, stats) {
  ...
  while (i < t.length) {
    ...
    // Missing handling + default fallback
    if (valueOut == null) {
      if (params.default != null && params.default !== '') {
        const defRaw = stripOuterQuotes(params.default);
        const defPath = resolveNestedInExpr(String(defRaw), scope, resolver);
        const defResolved = resolver.resolveScoped(defPath, scope, { default_ci: Boolean(params.ci) });
        valueOut = (defResolved != null) ? defResolved : defRaw;
      } else {
        // keep original directive text if unresolved and no default
        out += t.slice(a, end + 1);
        i = end + 1;
        continue;
      }
...
class Engine {
  constructor() {
    ...
    this.inlineRegistry = [
      ...
      new InlineGetDirective(),
      { apply: (line, scope, resolver, stats) => expandValuesAndGetInline(line, scope, resolver, stats) },
    ];
  }

  _checkLimits(depth) {
    if (depth > PDL.MAX_DEPTH) { this.stats.errors_parse++; return true; }
    if (this.stats.expansions > PDL.MAX_EXPANSIONS) { this.stats.errors_parse++; return true; }
    return false;
  }
