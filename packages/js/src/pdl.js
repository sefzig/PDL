...
function highlightConfig(before, after) {
  const b = before;
  const a = after;
  const enabled = !((b === '' && a === '') || (b === false && a === false));
  return { enabled, before: b, after: a };
}

function wrapHighlight(text, highlight, hlFlag = true) {
  if (!highlight || !highlight.enabled || hlFlag === false) return text;
  const before = (highlight.before == null) ? '' : String(highlight.before);
  const after = (highlight.after == null) ? '' : String(highlight.after);
  return `${before}${text}${after}`;
}

function applyHighlightHeuristics(text, highlight) {
  if (!highlight || !highlight.enabled) return text;
  const before = String(highlight.before ?? '');
  const after = String(highlight.after ?? '');
  const bEsc = escapeRegExp(before);
  const aEsc = escapeRegExp(after);
  let out = String(text);

  // Wrap markdown links/images: [text](url) or ![alt](url)
  const linkRe = new RegExp(`(\\!?\\[[^\\]]*\\]\\()\\s*${bEsc}(.*?)${aEsc}\\s*(\\))`, 'g');
  out = out.replace(linkRe, `${before}$1$2$3${after}`);

  // Wrap HTML attributes containing highlighted value: foo="...".
  const attrRe = new RegExp(`([A-Za-z_:][-A-Za-z0-9_:.]*\\s*=\\s*\"?)${bEsc}(.*?)${aEsc}(\"?)`, 'g');
  out = out.replace(attrRe, `${before}$1$2$3${after}`);

  return out;
}

function applyStringVariables(template, vars = {}, highlight = null) {
  const s = String(template || '');
  const hl = highlight || { enabled: false };
  let out = '';
  let depth = 0; // bracket depth for directives
    ...
    if (ch === '{') {
      ...
      if (close !== -1) {
        ...
        if (Object.prototype.hasOwnProperty.call(vars || {}, name)) {
          let val = (vars[name] == null) ? '' : String(vars[name]);
          if (depth === 0 && hl.enabled) val = wrapHighlight(val, hl, true);
          out += val;
          i = close;
          continue;
        }
...
class InlineIfHelper {
  ...
  static apply(line, scope, resolver, stats, highlight) {
  ...
  static apply(line, scope, resolver, stats, highlight) {
    ...
    while (true) {
      ...
      for (let k = 0; k < arr.length; k++) {
        ...
        if (!drop) {
          seg = InlineLoopExpander.apply(seg, childScope, resolver, stats, highlight);
          seg = expandValuesAndGetInline(seg, childScope, resolver, stats, highlight);
...
function expandValuesAndGetInline(text, scope, resolver, stats, highlight) {
  const t = String(text || '');
  ...
  while (i < t.length) {
    ...
    textOut = wrapHighlight(textOut, highlight, params.hl);
...
class Engine {
  constructor(highlight) {
    ...
    this.highlight = highlight || { enabled: false, before: '', after: '' };
    this.blockRegistry = [new LoopBlockDirective(), new IfBlockDirective()];
    this.inlineRegistry = [
      new InlineSetDirective(),
      { apply: (line, scope, resolver, stats, highlight) => InlineLoopExpander.apply(line, scope, resolver, stats, highlight) },
      { apply: (line, scope, resolver, stats, highlight) => expandValuesAndGetInline(line, scope, resolver, stats, highlight) },
    ];
  }
  ...
  expandLines(lines, scope, depth=0) {
    ...
    while (i < n) {
      ...
      for (const inline of this.inlineRegistry) {
        line = inline.apply(line, scope, this.resolver, this.stats, this.highlight);
...
class PDLParser {
  constructor(template, jsonRoot, { aliases = {}, variables = {}, highlight = { enabled: false, before: '', after: '' } } = {}) {
  ...
  render() {
    ...
    const engine = new Engine(this.highlight);
    ...
    const expandedLines = engine.expandLines(stripped.split('\n'), scope, 0)
      .map(line => expandValuesAndGetInline(line, scope, engine.resolver, engine.stats, this.highlight));
...
function render(template, data, options = {}) {
  const {
    headerIndentation = '#',
    dropFirstHeader = false,
    variables = {},
    hlBefore = null,
    hlAfter = null,
  } = options;

  const effectiveBefore = (hlBefore !== null && hlBefore !== undefined) ? hlBefore : PDL.HL_BEFORE;
  const effectiveAfter = (hlAfter !== null && hlAfter !== undefined) ? hlAfter : PDL.HL_AFTER;
  const highlight = highlightConfig(effectiveBefore, effectiveAfter);

  const jsonRoot = normalizeRoot(data);
  const templWithVars = applyStringVariables(String(template || ''), variables, highlight);
